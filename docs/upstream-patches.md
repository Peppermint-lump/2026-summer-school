...
- Expected behavior: the adapter sends commit-mode text over the DashScope realtime WebSocket, writes the returned 24 kHz WAV audio to the normal cache path, and never logs the API key. A missing `DASHSCOPE_API_KEY`, timeout, connection failure, or provider error automatically invokes the configured per-character Piper model. Sentence segments share an isolated reply-scoped TTS context: after the first Qwen failure, all remaining segments in that reply stay on Piper, while the next reply retries Qwen. This prevents a reply from switching Qwen -> Piper -> Qwen when only one parallel segment fails.
