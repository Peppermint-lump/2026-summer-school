...
def prepare_audio_payload(
    ...
    If audio_path is None, returns a short silent WAV payload. The frontend uses the
    audio playback path to apply Live2D actions, so this keeps expressions working
    while a TTS provider is unavailable.
    ...
    if not audio_path:
        # The web client applies expression actions during audio playback. Keep the
        # fallback long enough for the 250 ms Live2D fade-in to be visible.
        silent_duration_ms = 1500
        silent_audio = AudioSegment.silent(
            duration=silent_duration_ms, frame_rate=24000
        )
        buffer = io.BytesIO()
        ...
        return {
            ...
            "volumes": [0.0] * (silent_duration_ms // chunk_length_ms),
